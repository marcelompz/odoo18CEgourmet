# -*- coding: utf-8 -*-
"""
Created on 2025-11-18 13:23:06
...
class SaleOrderImportWizard(models.TransientModel):
    ...
    _description = 'Wizard to import sale order lines from Excel'

    sale_order_id = fields.Many2one(
        'sale.order', string='Orden de Venta', required=True, ondelete='cascade')
    excel_file = fields.Binary(
        string='Archivo Excel')
    file_name = fields.Char(
        string='Nombre del Archivo')
    sheet_index = fields.Integer(
        string="Número de pestaña")
    sheet_list_text = fields.Text(
        string="Pestañas detectadas", readonly=True)

    @api.onchange('excel_file')
    def _onchange_excel_file(self):
        self.sheet_list_text = ""
        self.sheet_index = False

        if not self.excel_file or not pd:
            return
        ...
        try:
            ...
            lines = []
            for i, name in enumerate(excel_data.sheet_names):
                lines.append(f"{i}: {name}")

            self.sheet_list_text = "\n".join(lines)

        except Exception:
            self.sheet_list_text = "No se pudieron leer las pestañas del archivo."
    ...
    def action_import_lines(self):
        ...
        try:
            ...
            try:
                if self.sheet_index is not None:
                    df = pd.read_excel(io.BytesIO(file_content), sheet_name=int(self.sheet_index))
                else:
                    df = pd.read_excel(io.BytesIO(file_content), sheet_name=0)
                    
            except Exception as e:
                raise UserError(_("Error al leer el archivo Excel. Detalle: %s") % e)
        ...
        for index, row in df.iterrows():
            ...
            existing_line = self.sale_order_id.order_line.filtered(lambda l: l.product_id == product and l.price_unit == precio_unitario)
            if existing_line:
                existing_line.write({
    ...
    def action_download_template(self):
        ...
        report_name = _('ImportarProductosEnVentas.xlsx')
